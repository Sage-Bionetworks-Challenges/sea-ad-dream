...
task2 <- syn$tableQuery(
  ...
  mutate(
    notes = if_else(evaluationid == "9617463", "Agent track submission", "")
  ) %>%
  select(-evaluationid)
...
kable(
  ...
  caption="Task 2 Final Round Submissions (Both Human and Agent Tracks)"
)
```
...
kable(
  bf %>%
  left_join(
    task2 %>% select(submitterid, notes),
